...
export default function ProductItem(props) {
  ...
  return (
    <li className="product-item outline-card p-8" key={props.index}>
      <Link to={`/product/${id}`} className="flex flex-col h-full">
        ...
        <div className="product-data text-center flex flex-col grow">
          <div className="product-title text-lg font-bold mb-2">
            {title}
          </div>
          <div className="product-price mb-4 grow">
            {formatPrice(price)}
          </div>
          <div className="product-link button">
            Read more
          </div>
        </div>
      </Link>
